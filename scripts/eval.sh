...
# Use --policy.path (not --policy.pretrained_path) so lerobot loads the
# FULL saved config from the checkpoint, including arch-shape fields like
# image_size that determine pos-grid dimensions. The .pretrained_path form
# synthesizes a fresh default config and only loads weights — which fails
# with size_mismatch when the trained image_size differs from the default.
